...
class BidsManager {
  ...
  static async createManualBid(auctionId, userId, amount) {
    // Create the new bid
  ...
  static async processAutoBids(auctionId, currentBid, currentBidderId) {
  ...
  static async calculateMinimumBids(auctionId, currentBid) {
    ...
    let minManualBid = currentBid + 1000; // Default increment
    ...
    if (autoBidSettings.length > 0) {
      ...
      if (autoBidSettings.length > 1) {
        ...
        minManualBid = Math.max(minManualBid, secondHighest.maxAmount + 1000);
      } else {
        // Only one auto-bid, just use current + increment
        minManualBid = Math.max(minManualBid, currentBid + 1000);
      }
