...
const createPasswordResetEmail = require("../../htmlPages/users/passwordResetEmail");
const createRealtorApplicationReceivedEmail = require("../../htmlPages/users/realtorApplicationReceivedEmail");
const createRealtorApprovedEmail = require("../../htmlPages/users/realtorApprovedEmail");
const createRealtorRejectedEmail = require("../../htmlPages/users/realtorRejectedEmail");
const createRealtorSuspendedEmail = require("../../htmlPages/users/realtorSuspendedEmail");
const PropertyRequest = require("../../model/property/propertyRequestModel");
const createRealtorRequestApprovedEmail = require("../../htmlPages/users/realtorRequestApprovedEmail");
const createRealtorRequestDeclinedEmail = require("../../htmlPages/users/realtorRequestDeclinedEmail");
...
// POST /api/v1/realtor/forgot/password   (public)
// Mirrors userController.forgotPassword, but looks up realtorModel and points
// the reset link back at the shared /set-new-password/:token page tagged with
// ?role=realtor so the frontend knows which account type to submit against.
exports.forgotPasswordRealtor = catchAsyncError(async (req, res, next) => {
  const { email } = req.body;
  const realtor = await realtorModel.findOne({ email: String(email || "").trim().toLowerCase() });
  if (!realtor) {
    return next(new Errorhandler("No realtor account found with this email", 404));
  }
  const resetPasswordtoken = realtor.getResetPasswordToken();
  await realtor.save({ validateBeforeSave: false });
  const resetUrl = `${process.env.CLIENT_URL}/set-new-password/${resetPasswordtoken}?role=realtor`;
  const passwordResetEmail = createPasswordResetEmail(realtor.name, resetUrl);
  try {
    sendEmail(realtor.email, realtor.name, "Email password recovery", passwordResetEmail);
    res.status(200).json({ success: true, message: "Recovery Email sent to realtor" });
  } catch (error) {
    realtor.resetPasswordToken = undefined;
    realtor.resetPasswordExpire = undefined;
    await realtor.save({ validateBeforeSave: false });
    return next(new Errorhandler(error.message, 500));
  }
});

// POST /api/v1/realtor/password/reset/:token   (public)
exports.resetPasswordRealtor = catchAsyncError(async (req, res, next) => {
  ...
  realtor.resetPasswordExpire = undefined;
  await realtor.save({ validateBeforeSave: false });
  return issueRealtorToken(realtor, 200, res, "Password reset successful");
});
