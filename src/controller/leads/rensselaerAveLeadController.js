...
const { scheduleRensselaerAveSignupCall } = require("../../services/calling/rensselaerAveCallScheduler");
const { enrichPerson } = require("../../services/shared/fullenrichService");
...
const getAllRensselaerAveLeads = catchAsyncError(async (req, res) => {
  ...
  const [leads, total] = await Promise.all([
    RensselaerAveLead.find(query).sort({ createdAt: -1 }).skip(skip).limit(limit).lean(),
    RensselaerAveLead.countDocuments(query),
  ]);
