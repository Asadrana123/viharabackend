...
const { getContractorsForCity } = require("../config/renovationContractorConstants");

class RenovationContractorService {
  static async findContractors(city, state, primaryArea) {
    const contractors = getContractorsForCity(city);
    return { contractors, source: 'verified' };
  }
}
